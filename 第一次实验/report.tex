...
\title{\textbf{编译器设计专题实验报告}\\[0.5em]\Large\color{themecolor}实验一：DFA模拟器实现}
...
\begin{webox}
本实验的DFA模拟功能已实现为Web前端页面，可在浏览器中直接操作，无需安装任何软件。

\textbf{主要功能：}五元组输入、DFA状态图可视化（基于vis.js库）、字符串接受/拒绝测试、长度$\leq N$的合法字符串BFS枚举。

\begin{center}
\includegraphics[width=0.95\textwidth,height=12cm,keepaspectratio]{image/web.png}
\end{center}

前端采用深色主题设计，输入区与可视化区左右分栏，状态图支持拖拽和缩放，交互体验较好。
\end{webox}
